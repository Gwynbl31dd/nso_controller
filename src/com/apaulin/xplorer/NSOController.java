...
public class NSOController {
	...
	private boolean transactionInProgress = false; //Verification if the transaction has been started prior to use any RPC request.
	...
	public NSOController(String address, String user, String password, int id) throws NSOException {
		...
		setReq(new RpcRequest(address + "/jsonrpc", user, password, getId()));
	}
	...
	public void init(String address, String user, String password) throws NSOException {
		...
		this.login = user;
		this.password = password;
		RpcRequest logReq = new RpcRequest(address + "/jsonrpc", user, password, getId());
		setReq(logReq);
	}
	...
	public void appendListEntry(String path, String value) throws NSOException, RPCException  {
		testTransaction();
		String result = req.send(new AppendListEntry(transactionId, path, value));
	...
	public String getLeafRefValues(String path) throws NSOException, RPCException {
		testTransaction();
		String result = req.send(new GetLeafRefValues(transactionId, path));
		return result;
	}
	...
	public String confirmCommit() throws NSOException, RPCException {
		testTransaction();
		String result = req.send(new ConfirmCommit(transactionId));
		return result;
	}
	...
	public int startTransaction(String db, String mode, String conf_mode, String tag, String on_pending_change) throws  NSOException, RPCException, RCPparameterException {
		String requestValue = req.send(new NewTrans(db, mode, conf_mode, tag, on_pending_change));
		int th = 0;
		try {
	...
	public String syncFrom() throws RPCException, NSOException {
		testTransaction();
		String result = req.send(new RunAction(transactionId, "/devices/sync-from"));
	...
	public boolean syncFrom(String deviceName) throws RPCException, NSOException {
		testTransaction();
		String result = req.send(new RunAction(transactionId, "/devices/device{"+deviceName+"}/sync-from"));
		JSONArray syncs = JsonPath.read(result, "$.result[*]");
	...
	public String runAction(String action) throws RPCException, NSOException {
		testTransaction();
		String result = req.send(new RunAction(transactionId, action));
		JSONArray syncs = JsonPath.read(result, "$.result[*]");
		String processed = new String();
		for (int i = 0; i < syncs.size(); i++) {
			String type = JsonPath.read(syncs.get(i), "$.name");
	...
	public String serviceCheckSync(String path) throws RPCException, NSOException {
		testTransaction();
		String result = req.send(new RunAction(transactionId, path + "/check-sync"));
		JSONArray syncs = JsonPath.read(result, "$.result[*]");
		String processed = new String();
		for (int i = 0; i < syncs.size(); i++) {
			String type = JsonPath.read(syncs.get(i), "$.name");
	...
	public String serviceDeepCheckSync(String path) throws RPCException, NSOException {
		testTransaction();
		String result = req.send(new RunAction(transactionId, path + "/deep-check-sync"));
		JSONObject error = ResultParser.parseError(result);
		// If no error
		if (error != null) {
			throw new RPCException(error.toString());
	...
	public String liveStatus(String device, String cmd, String action) throws NSOException, RPCException {
		testTransaction();
		String result = req.send(new RunAction(transactionId, "/devices/device{" + device + "}/live-status/" + action + "/any/", cmd));
		JSONArray syncs = JsonPath.read(result, "$.result[*]");
		String processed = new String();
		for (int i = 0; i < syncs.size(); i++) {
			String type = JsonPath.read(syncs.get(i), "$.name");
	...
	public String getNSOVersion() throws RPCException, NSOException, RCPparameterException {
		testTransaction();
		String parsedResult = ResultParser.parseStringResult(req.send(new GetSystemSetting(transactionId, "version")), "$.result");
		...
		return ResultParser.parseStringResult(req.send(new GetSystemSetting(transactionId, "version")), "$.result");
	}
